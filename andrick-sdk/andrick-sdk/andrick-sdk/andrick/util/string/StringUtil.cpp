...
namespace andrick
{
	const std::string StringUtil::msCLASS_NAME = "StringUtil";
	...
	std::string StringUtil::arrayToString(const std::string *array, GLuint size)
	{
		...
		if (!array || size == 0)
		{
			LoggerMaster::getUniversalLogger().logWarn(msCLASS_NAME, "The string array is null or empty. Return empty string.");
			str = "";
		}
		else
		{
	...
	std::string StringUtil::vectorToString(const std::vector<std::string> &array)
	{
		...
		if (array.empty())
		{
			LoggerMaster::getUniversalLogger().logWarn(msCLASS_NAME, "The string vector is empty. Return empty string.");
			str = "";
		}
		else
		{
