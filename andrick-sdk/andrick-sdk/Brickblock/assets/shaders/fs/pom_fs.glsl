...
{ 
	...
	vec3 c_original_texCoord_prime = mix(c_far, c_original_texCoord, uRange.y);
	vec3 c_far_prime = mix(c_far, c_original_texCoord, uRange.x);
...
//	float dt = (uRange.y - uRange.x) / n; //Mapped range
	...
	for (i = 0; i < numSamples; ++i)
	{
		...
		float height_prime = mix(uRange.x, uRange.y, height.r);
